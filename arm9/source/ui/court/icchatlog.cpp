...
void UICourtICChatLog::init()
{
	...
	u8* bgTiles = readFile("/data/ao-nds/ui/bg_icLog.img.bin", &bgTilesLen);
	u8* bgMap = readFile("/data/ao-nds/ui/bg_icLog.map.bin");
	u8* bgPal = readFile("/data/ao-nds/ui/bg_icLog.pal.bin");
