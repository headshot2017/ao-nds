...
void UICourtPair::init()
{
	...
	u8* bgTiles = readFile("/data/ao-nds/ui/bg_pair.img.bin", &bgTilesLen);
	u8* bgMap = readFile("/data/ao-nds/ui/bg_pair.map.bin");
	u8* bgPal = readFile("/data/ao-nds/ui/bg_pair.pal.bin");
